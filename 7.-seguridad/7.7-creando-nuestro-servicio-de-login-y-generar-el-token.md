...
{% code-tabs %}
{% code-tabs-item title="RolDAO.cs" %}
```csharp
public class RolDAO
{
...
Agreamos entre \[\] la palabra AllowAnonymous el cual le indica al servicio que permite el acceso a este servicio no requiere token, cualquiera puede acceder a este servicio.
...
1. Da clic en el botón Authorize que se encuntra arriba a la derecha.
