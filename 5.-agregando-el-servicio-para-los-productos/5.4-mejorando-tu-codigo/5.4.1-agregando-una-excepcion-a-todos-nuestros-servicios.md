...
{% code-tabs %}
{% code-tabs-item title="CustomMySQLException.cs" %}
```csharp
...
3. Sobreescribimos el método **OnException**

{% code-tabs %}
{% code-tabs-item title="CustomExceptionFilter " %}
```csharp
...
Podemos quitar el código **try** ... **catch** de nuestras clases DAO \(**CategoriaDAO y ProductoDAO**\) ya que ahora la excepción esta de forma general en nuestra clase CustomExceptionFilter.
