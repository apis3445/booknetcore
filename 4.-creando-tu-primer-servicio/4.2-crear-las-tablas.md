...
3. Tecleamos los nombres de los campos como propiedades públicas 
   ...
   ```csharp
   public class Categoria {     public int Id { get; set; }            public int Clave { get; set; }    public string Nombre { get; set; }}
   ```

4. Agregamos las validaciones de cada campo 
5. ```csharp
   public class Categoria{    public int Id { get; set; }            [Required(ErrorMessage = "Por favor teclea el campo Clave")]    [Range(1,999)]    public int Clave { get; set; }    [Required(ErrorMessage = "Por favor teclea el campo Nombre")]    public string Nombre { get; set; }}
   ```
...
```csharp
public class Categoria {     [Key]     [DatabaseGenerated(DatabaseGeneratedOption.Identity)]     public int Id { get; set; }            [Required(ErrorMessage = "Por favor teclea el campo Clave")]    [Range(1,999)]    public int Clave { get; set; }        [StringLength(80)]    [Required(ErrorMessage = "Por favor teclea el campo Nombre")]    [Column(TypeName = "VARCHAR(80)")]    public string Nombre { get; set; }}
```
{% endtab %}
{% endtabs %}
...
2.  Agregamos el constructor el cual recibe como parámetro un DBContextOptions el cual indicará las opciones de la base de datos a utilizar.
   ...
   ```csharp
   public class : DbContext{    public CaducaContext(DbContextOptions<CaducaContext> options) : base(options)    {    }      }
   ```
...
3. Agregamos la tabla como virtual DbSet  
   ...
   {    public CaducaContext(DbContextOptions<CaducaContext> options)                                              : base(options)    {    }    public virtual DbSet<Categoria> Categoria { get; set; }}
   ```
...
```javascript
{    "ConnectionStrings": {     "DefaultConnection":"server=localhost;port=3306;database=caduca;user=AdminCaduca;Password=StKRV6MR6A;sslMode=none"   },   "Logging": {     "LogLevel": {           "Default": "Warning"     }   },   "AllowedHosts": "*"}
```
{% endtab %}
{% endtabs %}
...
```csharp
public void ConfigureServices(IServiceCollection services) {       services.AddMvc().SetCompatibilityVersion(   CompatibilityVersion.Version_2_2);    services.AddDbContext<CaducaContext>(options =>                   options.UseMySql(Configuration                         .GetConnectionString("DefaultConnection"))); }
```
{% endtab %}
{% endtabs %}
